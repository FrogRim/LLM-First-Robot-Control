...
본 프로젝트는 **자연어 명령으로부터 물리 속성 기반 제어 매개변수를 직접 생성하는 LLM 기반 로봇 제어 시스템**입니다. 규칙 기반, 강화학습 기반, LLM 기반 방식을 비교하여 우수성을 검증하였으며, **Genesis 시뮬레이터**를 통해 실험을 수행했습니다.

또한, **DROID 공개 데이터셋을 Genesis AI + Franka Panda 환경으로 변환**하고, **LLM-First 아키텍처**를 통해 자연어 명령에서 물리 속성을 추출하여 로봇 제어 파라미터를 생성하는 **완전한 End-to-End 시스템**입니다.
